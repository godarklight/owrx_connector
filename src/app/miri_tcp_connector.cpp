#include "miri_tcp_connector.hpp"
...
int main (int argc, char** argv) {
    Connector* connector = new MiriTcpConnector();
    return connector->main(argc, argv);
}

uint32_t MiriTcpConnector::get_buffer_size() {
    return miri_buffer_size;
}

std::stringstream MiriTcpConnector::get_usage_string() {
...
std::vector<struct option> MiriTcpConnector::getopt_long_options(){
...
int MiriTcpConnector::receive_option(int c, char* optarg) {
    switch (c) {
        case 'b':
            bias_tee = true;
            break;
...
int MiriTcpConnector::parse_arguments(int argc, char** argv) {
...
void MiriTcpConnector::print_version() {
    std::cout << "miri_tcp_connector version " << VERSION << std::endl;
    Connector::print_version();
}

int MiriTcpConnector::open() {
    ...
    if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        std::cerr << "miri_tcp socket creation error: " << sock << std::endl;
        return 1;
    }
    ...
    if (connect(sock, (struct sockaddr *)&remote, sizeof(remote)) < 0) {
        fprintf(stderr, "miri_tcp connection failed\n");
        return 2;
    }
    return 0;
}

int MiriTcpConnector::send_command(struct command cmd) {
    ssize_t len = sizeof(cmd);
    ssize_t sent = send(sock, &cmd, len, 0);
    return len == sent ? 0 : -1;
}

int MiriTcpConnector::setup() {
    int r;
...
int MiriTcpConnector::read() {
    ssize_t bytes_read;
    int16_t* buf = (int16_t*) malloc(miri_buffer_size);

    while (run) {
        bytes_read = recv(sock, buf, miri_buffer_size, 0);
        if (bytes_read > 0) {
            processSamples(buf, bytes_read / 2);
        } else {
            fprintf(stderr, "error or no data on miri_tcp socket, closing connection\n");
            break;
        }
    }
...
int MiriTcpConnector::close() {
    return ::close(sock);
}

void MiriTcpConnector::applyChange(std::string key, std::string value) {
    int r = 0;
...
int MiriTcpConnector::set_center_frequency(double frequency) {
    return send_command((struct command) {0x01, htonl(frequency)});
}

int MiriTcpConnector::set_sample_rate(double sample_rate) {
    return send_command((struct command) {0x02, htonl(sample_rate)});
}

int MiriTcpConnector::set_gain(GainSpec* gain) {
...
int MiriTcpConnector::set_ppm(double ppm) {
    return send_command((struct command) {0x05, htonl((int32_t) ppm)});
}

int MiriTcpConnector::set_direct_sampling(int direct_sampling) {
...
int MiriTcpConnector::set_bias_tee(bool bias_tee) {
